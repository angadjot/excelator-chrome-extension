...
exports.onFileSelection = (file, config) => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();

    reader.addEventListener('loadend', () => {
      let json = JSON.parse(reader.result);
      resolve(onLoadEvent(json, reader, config));
    });
    reader.readAsText(file);
  });
};

const onLoadEvent = (json, reader, config) => {
  let confs = [];
  for (const [sheetName, sheetValue] of Object.entries(json)) {
    let conf = prepareJson(sheetValue, config);
    conf.name = sheetName;
...
const prepareJson = function (json, config) {
  let res = {};
  let conf = config || {};
  let jsonArr = [].concat(json);
